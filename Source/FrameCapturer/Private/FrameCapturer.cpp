...
void FFrameCapturer::PrepareCapturingFrames_MainThread(uint32 InFrameCount, TFunction<void(FRHICommandListImmediate&, const TArray<FColor>&, const FTexture2DRHIRef&, int32, int32)>&& InFrameReady)
{
...

	{
		...
		if (ViewportResource)
		{
			ViewportResourceCache = *((FViewportRHIRef*)ViewportResource);
#if PLATFORM_ANDROID
			RHISetPendingRequestAndroidBackBuffer(ViewportResourceCache, true);
#endif
		}
	}
...
#if PLATFORM_ANDROID
	RHISetPendingRequestAndroidBackBuffer(ViewportResourceCache, false);
#endif
}
...
{
#if PLATFORM_ANDROID
	if (!RHIIsRequestAndroidBackBuffer(ViewportResourceCache.GetReference()))
	{
		return;
	}
#endif
